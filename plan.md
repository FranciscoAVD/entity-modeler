...
Selecting a Space/Orbit/Node result flies the camera to it, same as a single click on a sidebar row or 3D-scene object — it does not open a tab (opening a tab requires a double-click in the 3D scene, or the sidebar row's "View notes" context-menu item). Selecting a Tag result instead opens TagObjectsDialog, listing the objects that carry it — picking one of *those* does open a tab (openTab, not focusOn), the one exception to "search results are camera-only"
...
**Follow-up, post-Phase-7 — the overview distance scales with the project's space count.** User request: the "reset view"/no-active-tab overview previously sat at one fixed distance regardless of project size — same zoom for a 1-space project as a 20-space one. `cameraFocus.ts`'s `overviewDistance(spaceCount)` now computes `clamp(MIN, MIN + spaceCount * PER_SPACE, MAX)` (20 / 6 / 70 respectively — MIN alone matches the old fixed distance almost exactly, so the common 1-2-space case doesn't visibly change; MAX stays comfortably under OrbitControls' own hard `maxDistance={80}`). `resolveCameraFocus` gained a `projectId` parameter (threaded from `Scene.tsx` → `CameraRig.tsx`) to look up the count via the existing `spacesInProject` selector. Caught and fixed along the way: switching projects doesn't bump `resetViewToken` or touch `activeTabId` (tabs aren't project-scoped), so two projects idling at the same reset token/no-tab fallback would have produced identical `CameraFocus.key` values — `CameraRig`'s `focus.key === focusKeyRef.current` bail-out would then have silently skipped re-tweening after a project switch, leaving the camera at the *previous* project's distance. Fixed by folding `projectId` into the default-focus key itself, not just using it as a distance input.
...
**Follow-up tuning, same session — spaces specifically need more room than that.** `layoutGroup` gained an optional 4th param, `separationPadding` (defaults to the shared `SEPARATION_PADDING`), so a call site can ask for more room than the default without affecting other tiers. The spaces-within-project call now passes `SPACE_SEPARATION_PADDING` (8, vs. the shared 2.5) — spaces are what a user pans/zooms across to get their bearings in the whole project, so visually distinct gaps matter more there than at the tighter, already-zoomed-in orbit/node tiers.
