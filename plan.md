...
Explicitly not designed yet: migrations/actual DB file setup, how the store's existing mutations (addNode, updateNodeTags, etc.) sync back to the server (this pass only covers reads — write endpoints are a separate design pass), auth (single-user constraint, decision #14, still holds)
