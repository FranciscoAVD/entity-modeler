...
export function SidebarTree({ projectId, onRequestCreate }: TreeProps & { projectId: string }) {
  const spaces = useModelStore(useShallow((state) => spacesInProject(state, projectId)));

  return (
    <div className="space-y-2 text-sm">
      {spaces.map((space) => (
        <SpaceRow key={space.id} space={space} onRequestCreate={onRequestCreate} />
      ))}
    </div>
  );
}
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuCheckboxItem checked={visible} onCheckedChange={onToggleVisible}>
          Visible
        </DropdownMenuCheckboxItem>
        <DropdownMenuSeparator />
        {children}
...
function SpaceRow({ space, onRequestCreate }: TreeProps & { space: Space }) {
  ...
  return (
    <div>
      <div className={`flex items-center gap-2 font-medium ${hidden ? "text-muted-foreground" : ""}`}>
        <SpaceIcon className="size-3.5 shrink-0" />
        <span className="min-w-0 flex-1 truncate">{space.label ?? space.name}</span>
        <OptionsMenu
          label={space.name}
          visible={!hidden}
          onToggleVisible={() => toggleSpaceVisibility(space.id)}
        >
          <DropdownMenuItem onSelect={() => onRequestCreate({ type: "orbit", spaceId: space.id })}>
            <OrbitIcon className="mr-1.5 size-3.5" />
            Add orbit
          </DropdownMenuItem>
          <DropdownMenuItem onSelect={() => onRequestCreate({ type: "node", spaceId: space.id })}>
            <EntityIcon className="mr-1.5 size-3.5" />
            Add node
          </DropdownMenuItem>
        </OptionsMenu>
      </div>
      <div className="mt-1.5 ml-5 space-y-1.5">
        {orbits.map((orbit) => (
          <OrbitRow key={orbit.id} orbit={orbit} onRequestCreate={onRequestCreate} />
        ))}
      </div>
    </div>
  );
...
function OrbitRow({ orbit, onRequestCreate }: TreeProps & { orbit: Orbit }) {
  ...
  return (
    <div className="text-muted-foreground flex items-center gap-2">
      ...
      <span className="min-w-0 flex-1 truncate">{orbit.label ?? orbit.name}</span>
      <OptionsMenu
        label={orbit.name}
        visible={!hidden}
      ...
      >
        <DropdownMenuItem
          onSelect={() => onRequestCreate({ type: "node", spaceId: orbit.spaceId, orbitId: orbit.id })}
        >
          <EntityIcon className="mr-1.5 size-3.5" />
          Add node
        </DropdownMenuItem>
